...
追赶法：
$$
\text{则求}Ax=f\text{等价于求}\begin{cases}Ly=f\\Ux=y\end{cases} \\
其中f=\left(b_{1},f_{2},\ldots,f_{n}\right)^{T},\text{故有} \\
\text{1} \\
...
\text{解得}\begin{cases}y_1=f_1\\y_i=f_i-p_iy_{i-1}\end{cases}\left(i=2,\ldots,n\right)\\
\text{再由}\begin{bmatrix}q_1&c_1&&&\\&q_2&c_2&&\\&&\ddots&\ddots&\\&&&q_{n-1}&c_{n-1}\\&&&&q_n\end{bmatrix}\begin{bmatrix}x_1\\x_2\\\vdots\\x_{n-1}\\x_n\end{bmatrix}=\begin{bmatrix}y_1\\y_2\\\vdots\\y_{n-1}\\y_n\end{bmatrix}\\\text{解得}\quad\begin{cases}\quad x_n=\frac{y_n}{q_n}\\\quad x_n=\frac{y_n}{q_n}\\\quad x_i=\frac{y_i-c_ix_{i+1}}{q_i}&(i=n-1,\ldots,1)&\end{cases}\\\text{以上称为解三对 角方程组的道赶法。}
$$
