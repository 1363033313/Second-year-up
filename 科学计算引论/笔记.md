...
**Newton-Cotes公式如下：**
$$
\int_a^bf(x)\mathrm{d}x\approx(b-a)\sum_{n=0}^NB_nf(a+nh),B_n=\frac{(-1)^{N-n}}{N[n!~(N-n)!]}\int_0^N\prod_{i=0,i\neq n}^N(t-i)\mathrm{d}t,
$$
...
$$
可以看出，待定参数$A_k,x_k\left(k=0,1,\cdots n\right)$共$2n+2$ 个，如果令$f\left(x\right)=1,x,x^2,\cdots,x^{2n+1}$使求积公式(5. 精确成立，即$R[f]=0$,从理论上看，建立了$求A_{k,}x_k\left(k=0,1,\cdotp\cdotp\cdotp n\right)$的$2n+2$个代数方程组
